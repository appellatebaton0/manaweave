...
## Door orientations
enum ORI{NORTH, EAST, SOUTH, WEST}

## Stores the orientation of the door.
...
func corresponds_with(with:DoorBit) -> bool:
	# Log the orientations in an array.
	# This decouples the orientation from the door,
	# so NORTH & SOUTH and SOUTH & NORTH pairs are
	# read the same, for example.
	var oris = [false, false, false, false]
	
	oris[orientation] = true
	oris[with.orientation] = true
	
	# Return whether there's a valid pair.
	return (oris[ORI.NORTH] and oris[ORI.SOUTH]) or (oris[ORI.WEST] and oris[ORI.EAST])
